...
GeneratedTelTogglePref::GeneratedTelTogglePref(Profile* profile)
    : profile_(profile) {
  user_prefs_registrar_.Init(profile->GetPrefs());
  user_prefs_registrar_.Add(
      browsing_data::prefs::kTaktakTelEnabled,
      base::BindRepeating(&GeneratedTelTogglePref::OnSourcePreferencesChanged,
                          base::Unretained(this)));
}

GeneratedTelTogglePref::~GeneratedTelTogglePref() = default;

extensions::settings_private::SetPrefResult GeneratedTelTogglePref::SetPref(
    const base::Value* value) {
  if (!value->is_bool()) {
    return extensions::settings_private::SetPrefResult::PREF_TYPE_MISMATCH;
  }
  profile_->GetPrefs()->SetBoolean(browsing_data::prefs::kTaktakTelEnabled,
                                   value->GetBool());
