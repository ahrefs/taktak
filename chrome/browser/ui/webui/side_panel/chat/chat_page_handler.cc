...
constexpr char kNewChatEvent[] = "new_chat";
constexpr char kNewChatWithPageContextEvent[] = "new_chat_with_page_context";
constexpr char kNewChatWithoutPageContextEvent[] =
...
std::string_view GetEventNameFromActionType(
    chat::mojom::ActionType action_type) {
  switch (action_type) {
    case chat::mojom::ActionType::SUMMARIZE_PAGE:
      return "summarize";
    case chat::mojom::ActionType::EXPLAIN:
      return "explain";
    case chat::mojom::ActionType::FACT_CHECK:
      return "fact_check";
    case chat::mojom::ActionType::TRANSLATE:
      return "translate";
    case chat::mojom::ActionType::DRAFT_SOCIAL_MEDIA_POST:
      return "draft_a_post";
    default:
      return "unknown";
  }
...
class ChatHistoryCache {
 public:
  ...
  chat::mojom::ChatStatePtr GetChatState() {
    ...
    chat_state->conversations =
        std::vector<chat::mojom::SavableConversationModelPtr>();
  ...
  chat::mojom::SiteInfoPtr GetSiteInfo() {
    ...
    if (this->site_info_) {
      ...
      site_info->is_content_usable_in_conversations =
          this->site_info_->is_content_usable_in_conversations;
    }
 ...
 private:
  ...
  base::Lock lock_;
  std::unordered_map<std::string, chat::mojom::SavableConversationModelPtr>
      chat_cache_;
...
ChatPageHandler::ChatPageHandler(
  ...
  scoped_refptr<network::SharedURLLoaderFactory> completion_url_loader_factory =
      profile_->GetDefaultStoragePartition()
          ->GetURLLoaderFactoryForBrowserProcess();
  api_client_ = std::make_unique<CompletionApiClient>(
      std::move(completion_url_loader_factory));

  scoped_refptr<network::SharedURLLoaderFactory> cs_url_loader_factory =
      profile_->GetDefaultStoragePartition()
          ->GetURLLoaderFactoryForBrowserProcess();
  cs_handler_ =
      std::make_unique<cs_handler::CSHandler>(std::move(cs_url_loader_factory));
...
void ChatPageHandler::SetSiteInfo(chat::mojom::SiteInfoPtr site_info,
  ...
  page_content_extractor_helper_ =
      std::make_unique<PageContentExtractorHelper>(contents);
...
void ChatPageHandler::GetChatState(GetChatStateCallback callback) {
  std::move(callback).Run(
      ChatHistoryCache::GetInstance()->GetChatState().Clone());
}

void ChatPageHandler::SaveConversation(
    chat::mojom::SavableConversationModelPtr conversation) {
  ChatHistoryCache::GetInstance()->SaveConversation(conversation.Clone());
}

void ChatPageHandler::GetSiteInfoFromCache(
    GetSiteInfoFromCacheCallback callback) {
  chat::mojom::SiteInfoPtr site_info =
      ChatHistoryCache::GetInstance()->GetSiteInfo();
  DCHECK(site_info);
  std::move(callback).Run(site_info.Clone());
}
...
void ChatPageHandler::SaveThinkingState(bool thinking_state) {
  ...
  prefs->SetBoolean(browsing_data::prefs::kChatThinkingEnabled, thinking_state);
}
...
void ChatPageHandler::OnPageContentExtracted(
  ...
  if (action_type != chat::mojom::ActionType::NONE) {
    ...
    auto md = c.convert();
    max_content = md;
    DVLOG(1) << __func__ << " |>> markdown content -> " << max_content;
  }
...
void ChatPageHandler::SubmitQuery(
    chat::mojom::ActionType action_type,
...
void ChatPageHandler::OpenURL(const std::string& url,
