...
void APIRequestHelper::URLLoaderHandler::OnDataReceived(
  ...
  if (is_sse_) {
    ...
    ScopedPerfTracker tracker("APIRequestHelper.OnDataReceivedNoSSE");
...
void APIRequestHelper::URLLoaderHandler::OnParseJsonResponse(
  ...
  ScopedPerfTracker tracker("APIRequestHelper.ProcessResultOnUI");
  ...
  for (const auto& data : stream_data) {
    ...
    auto on_json_parsed =
        [](base::WeakPtr<APIRequestHelper::URLLoaderHandler> handler,
          ...
          ScopedPerfTracker tracker("APIRequestHelper.ParseSSECallback");
